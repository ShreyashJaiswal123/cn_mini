...
def startNetwork():
    ...
    topo = MyTopo()
    # open ryu-controller and run ifconfig in that terminal 
    # set RemoteController ip to ryu-controller ip from above command
    c0 = RemoteController('c0', ip='192.168.0.101', port=6653)
    net = Mininet(topo=topo, link=TCLink, controller=c0)

    net.start() # starting mininet topology
    ...
    sleep(100) #sleep so that controller can show output updated
